...
void led_init ( void )												// Inicializa̤̣o
{
		...
		RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC, ENABLE);		// Ativar clock da GPIO
		...
		GPIO_Init(GPIOC, &gpio);									// Inicializa̤̣o da GPIO

}

void led_write(uint16_t GPIO_Pin, BitAction BitVal)					// Escrita no led
{
	GPIO_WriteBit(GPIOC, GPIO_Pin, BitVal);							// Escreve no Pino GPIO_Pin o valor BitVal
}
